...
RSpec.describe Circuitdata::JsonValidator::JsonSchemaErrorParser do
  ...
  it "handles min values" do
    errors = generate_errors_and_translate(
      { test: 1 },
      { type: "object", properties: { test: { type: "number", minimum: 2 } } }
    )
    expect(errors).to eql([{ :source_path => "/test", :field => "test", :problem => "min_not_met", expected: "2" }])
  end

  it "handles min values" do
    errors = generate_errors_and_translate(
      { test: 3 },
      { type: "object", properties: { test: { type: "number", maximum: 2 } } }
    )
    expect(errors).to eql([{ :source_path => "/test", :field => "test", :problem => "max_exceeded", expected: "2" }])
  end
